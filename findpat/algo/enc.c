...
uint fa_copy_cont(uchar* fasrc, uchar* dst, uint n) {
	...
	return fa_copy_cont_sep(fasrc, dst, n, _LCP_SEPARATOR); // Uses
	#else
	return fa_copy_cont_sep(fasrc, dst, n, 0x0a); // Uses 0x0a as separator, it's included anyway
	#endif
}

uint fa_copy_cont_sep(uchar* fasrc, uchar* dst, uint n, const uchar sep) {
	uint res = 0;
	uint inv = 0;
	uchar* src = fa_strip_header(fasrc, n);
	n -= (src-fasrc);
	while(n--) {
		if ((*src != 0x0a)&&(*src != 'A')&&(*src != 'C')&&(*src != 'G')&&(*src != 'T')&&(*src != 'N')&&(*src != sep)) {
			inv++;
			if (inv < 5) {
				fprintf(stderr, "Warning: Invalid char found on FASTA stream: '%c' (0x%2X).\n", *src, *src);
			} else if (inv == 5) {
				fprintf(stderr, "Warning: More invalid chars found in the same FASTA stream (omitted).\n");
			}
		} else {
...
uint trac_convert_pos_real_to_virtual(uint x, uint* trac_buf, uint trac_size) {
	...
	if (b > 0 && trac_buf[2 * b] + trac_buf[2 * b - 1] <= x) {
		/* Gap */
		return -1;
	} else {
...
/* Computes the reverse-complementary of src */
void rev_comp(uchar* src, uint n) {
	#define _rev_comp(X) ((X)=='A'?'T':((X)=='T'?'A':((X)=='C'?'G':((X)=='G'?'C':(X)))))
	uint m = n/2, i;
	uchar x;
	for(i=0; i<m; i++) {
		x = _rev_comp(src[i]);
		src[i] = _rev_comp(src[n-1-i]);
		src[n-1-i] = x;
	}
	if (n%2) src[m] = _rev_comp(src[m]);
}
