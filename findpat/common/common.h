...
/** Integer logarithm **/
inline uint bneed(uint x) __attribute__((const));

void endian_swap_16(ushort* x);
...
char *fileReadline(FILE *in, uint *n);
...
#else
inline uint bneed(uint x) { uint res = 0; while (x) res++,x>>=1; return res; }
#endif
